...
class TopicModeler:
    ...
    def get_topic_info(self):
        """
        Возвращает DataFrame с информацией о темах, включая автоматически сгенерированные имена.
        """
        if self.topic_model:
    ...
    def get_topic_names_map(self):
        """
        Возвращает словарь, сопоставляющий ID темы с ее именем.
        Использует автоматически сгенерированные имена от BERTopic.
        """
        if self.topic_model:
            topic_info = self.topic_model.get_topic_info()
            # Создаем словарь: {Topic ID: Topic Name}
            # Для Topic -1 (шумовые) присваиваем имя 'Без темы'
            topic_names = {
                row['Topic']: row['Name'] if row['Topic'] != -1 else 'Без темы'
                for _, row in topic_info.iterrows()
            }
            logger.info("Карта названий тем успешно сгенерирована.")
            return topic_names
        logger.warning("Модель BERTopic не инициализирована или не обучена для получения названий тем.")
        return {}
...
if __name__ == '__main__':
    ...
    #  НОВОЕ: Получение карты названий тем 
    topic_names_map = topic_modeler.get_topic_names_map()
    logger.info(f"Карта названий тем: {topic_names_map}")
    ...
    df_filtered['topic_id'] = topics # Переименовал колонку, чтобы избежать путаницы с 'topic' как названием
    df_filtered['topic_probability'] = probabilities

    #  НОВОЕ: Добавление колонки с названиями тем 
    df_filtered['topic_name'] = df_filtered['topic_id'].map(topic_names_map)
    logger.info("Добавлены названия тем к DataFrame.")


    # Сохранение DataFrame с сообщениями, их предобработанными версиями, хештегами и темами
    try:
        df_filtered[['message_id', 'text', 'processed_text', 'extracted_hashtags', 'topic_id', 'topic_name', 'topic_probability']].to_csv("messages_with_topics.csv", index=False, encoding='utf-8')
        logger.info("Сообщения с присвоенными темами и названиями сохранены в messages_with_topics.csv")
    except Exception as e:
    ...
    try:
        ...
        if num_unique_topics > 0:
            top_n_for_barchart = min(5, num_unique_topics)
